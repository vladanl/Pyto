...
categories = ['ctrl', 'cond_x', 'cond_y']

# experiment identifiers
identifiers = [
    'ctrl_108', 'ctrl_109', 'ctrl_110', 'ctrl_114',
    'ctrl_202', 'ctrl_204', 'ctrl_206', 'ctrl_209',
    'ctrl_327', 'ctrl_328', 'ctrl_329', 'ctrl_330',
    'ctrl_421', 'ctrl_428',

    'cond_x_115', 'cond_x_121', 'cond_x_122', 'cond_x_221',
    'cond_x_223', 'cond_x_224a', 'cond_x_224b', 'cond_x_227',

    'cond_y_102', 'cond_y_103', 'cond_y_104', 'cond_y_105',
    'cond_y_211', 'cond_y_212', 'cond_y_216', 'cond_y_219',
    'cond_y_301', 'cond_y_303', 'cond_y_306', 'cond_y_310', 'cond_y_311'
]
...
reference.update(
    {'0-5': '0-5', '5-10': '0-5', '10-15': '0-5', '15-20': '0-5',
     '10-20': '0-5', '20-100': '0-5',
...
def main(individual=False, save=False, analyze=False):
    ...
    util.plot_layers(data=layer, mode='all', pp=pp, groups=categories,
                identifiers=identifiers)

    # plot individual vesicle occupancy with means
    plot_layers(data=layer, mode='all&mean', pp=pp,
                groups=categories, identifiers=identifiers)

    # plot individual vesicle occupancy with means
    plot_layers(data=layer, mode='mean', pp=pp, groups=categories,
    ...
    # radius of bulk svs
    stats(data=bulk_sv, name='radius_nm', join='join', pp=pp, groups=categories,
    ...
    stats(data=clust, name='fract_bound_max', join='join', pp=pp,
          groups=categories, identifiers=identifiers, reference=reference,
    ...
    stats(data=conn, name='length_nm', join='join', pp=pp, groups=categories,
    ...
    stats(data=near_sv, name='n_tether', join='join', pp=pp, groups=categories,
          identifiers=identifiers, reference=reference, test='kruskal',
          title='N tethers per proximal sv', y_label='N tethers')

    # n tethers per tethered sv
    stats(data=near_teth_sv, name='n_tether', join='join', pp=pp,
          groups=categories, identifiers=identifiers, reference=reference,
          test='kruskal',
    ...
    # tether length
    # Q: Shouldn't length use t-test?
    stats(data=tether, name='length_nm', join='join', pp=pp, groups=categories,
          identifiers=identifiers, test='kruskal', reference=reference,
    ...
    # surface of the AZ
    stats(data=near_sv, name='az_surface_um', join='join', pp=pp,
    ...
    stats(data=near_sv, name='az_surface_um', join=None, pp=pp,
          groups=categories, identifiers=identifiers, test='t',
          reference=reference,
          y_label=r'AZ area [${\mu}m^2$]', title='AZ surface area')

    # N proximal svs per synapse
    stats(data=near_sv, name='n_vesicle', join='join', pp=pp,
          groups=categories, identifiers=identifiers, test='t',
    ...
    stats(data=tether, name='n_tether', join='join', pp=pp, groups=categories,
          identifiers=identifiers, test='t', reference=reference,
