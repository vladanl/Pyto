...
class BoundaryNormal:
    ...
    def find_normals(self, points=None, raw_all_points=True):
        ...
        self.dist_abs, self.dist_vector, _, self.points, self.points_good = \
            vector_filter_res
    ...
    def setup_vector_filter(
        ...
        Point neighborhood is defined by (arg) dist_kernel.
        ...
        Returns:
          - dist_abs: (list of length n points) where elements correspond to
          points, each element is a 1d ndarray of length n_neighbors
          containing absolute values of vectors that are located in the
          neighborhood of the point
          - dist_vector: (list of length n_points) where elements correspond
          to points, each element is 2d ndarray (n neighbor points x ndim)
          containing vectors that are located in the neighborhood
          of the point
          - coordinates: (list of length n_points) where elements correspond
          to points, each element is 2d ndarray (n neighbor points x ndim)
          containing coordinates the distance vectors (specified by
          dist_abs and dist_vector) locations 
        ...
        im_empty = pyto.grey.Image()
        coords = np.indices(vector_abs.shape)
        ...
        for po in points:
            ...
            image_inset_adj = im_empty.findIntersectingInset(
                inset=image_full, inset2=image_inset)
            image_adj_origin = [x.start for x in image_inset]
            kernel_inset_adj = [
                slice(im_slice.start - orig, im_slice.stop - orig) 
                for im_slice, orig in zip(image_inset_adj, image_adj_origin)]

            # extract common insets of vectors, coords and kernel
            ...
            coords_adj =  np.asarray(
                [co[*image_inset_adj] for co in coords])
            kernel_adj = dist_kernel[*kernel_inset_adj]

            # select coords, abs and vector distances by existing distances
            ...
            hood_coords_one = np.asarray(
                [co[mask] for co in coords_adj]).transpose()

            # select coords in the same way
            #hood_coords_one = np.transpose(mask.nonzero())
            ...
            points_res.append(po)
            points_good.append(True)
            hood_coords.append(hood_coords_one)

        return (dist_abs, dist_vector, hood_coords,
    ...
    def distance_weighted_sum(
        ...
        different weighting. For example:
          ...
          - gamma > 0: regularization factor, gives the highest
