...
class TestSet(np_test.TestCase):
    ...
    def test_get_tomo_path(self):
        ...
        particles = Set(work_path=common.work_path_abs)
    ...
    def test_adjust_box_coordinates(self):
        ...
        # test
        particles = Set(work_path=common.work_path_abs, box_size=6)
    ...
    def test_write_particles(self):
        ...
        ToDo: check that pixel size preserved
        """
        ...
        particles.write_particles(identifier='exp_1', pixelsize=1.23)
        desired_paths = [
        ...
        # test pixel size
        for ind in list(range(3)):
             np_test.assert_almost_equal(images[ind].pixelsize, 1.23)
        #articles.pixelsize = 3.45
        particles.write_particles(identifier='exp_1', pixelsize=3.45)
        images = [Image.read(file=path) for path in desired_paths]
        for ind in list(range(3)):
            np_test.assert_almost_equal(images[ind].pixelsize, 3.45)
    ...
    def test_extract_particles(self):
        ...
        particles, labels = Set.extract_particles(
            ...
            work_path=common.work_path_abs, segment_var=common.catalogs_var, 
        ...
        np_test.assert_equal((actual_paths_y == desired_paths).all(), True)
       
        # test particle tomo pixel size (set from tomo)
        images = [Image.read(file=path) for path in actual_paths_x]
        for ind in list(range(3)):
             np_test.assert_almost_equal(images[ind].pixelsize, 1.23)     
        images = [Image.read(file=path) for path in actual_paths_y]
       ...
       # test label tomo paths
        ...
        # test label tomo pixel size (set from tomo)
        images = [Image.read(file=path) for path in actual_paths_x]
        for ind in list(range(3)):
             np_test.assert_almost_equal(images[ind].pixelsize, 1.23)     
        images = [Image.read(file=path) for path in actual_paths_y]
        ...
        with np_test.assert_raises(ValueError):
            particles, labels = Set().extract_particles(
                struct=common.tethers, work=common.work,
                work_path=common.work_path_abs,
                segment_var=common.catalogs_var,
                group_names=['group_x', 'group_y'],
                identifiers=['exp_3', 'exp_17', 'exp_2'], 
                box_size=6, particle_dir=common.particle_dir_abs)

        # check pixel when set manually for particle and label tomos
        particles, labels = Set.extract_particles(
            struct=common.tethers, work=common.work,
            work_path=common.work_path_abs, segment_var=common.catalogs_var, 
            group_names=['group_x', 'group_y'],
        ...
        for ind in list(range(3)):
             np_test.assert_almost_equal(
                 images[ind].pixelsize, 5.67, decimal=4)     
        images = [Image.read(file=path) for path in actual_paths_y]
        for ind in list(range(2)):
             np_test.assert_almost_equal(
                 images[ind].pixelsize, 5.67, decimal=4)     
