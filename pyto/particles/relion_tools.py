...
def extract_random(starfile, out, number, tablename='data_images'):
    ...
    particle_indices = np.asarray(
        random.sample(list(range(n_particles)), number))
...
def print_class_summary(
    ...
    sorted_iters = np.sort(list(class_content.keys()))
...
def get_n_particle_class_change(
    ...
    if out == 'dict':
        ...
        sorted_iters = np.sort(list(result_dict.keys()))
...
def two_way_class(
    ...
    if isinstance(iters, tuple):
        ...
        iters_class = iters
        iters_group = iters
    if isinstance(iters_class, (int, int)):
        iters_class = [iters_class]
    if isinstance(iters_group, (int, int)):
...
def get_array_data(starfile, tablename, labels=None, types=float):
    ...
    Returns dictionary where keys are labels and values are data arrays 
    """
...
def write_table(
        starfile, labels, data, format_, comment='#', header=None, 
        tablename=None, labelindent=None, delimiter=' ', append=False,
        encoding='ascii'):
    """
    ...
    order of variables in the star file.
    
    The format arg determines the formatting of variables for writting 
    the star file. It has to correspond exactly to args labels and data,
    except if format is 'auto'. In that case, the data type is determined 
    from the data directly and the default formats are used ('%12.6f' '%12d').  

    Typically, args starfile, tablename, labels, data, and format_ are given.
    ...
    Arguments:
      ...
      - format_: write data format, can be one of the following:
        - single format string: '%6d %s %8.3f', where the number of '%'s has
        to be the same as the number of labels
        - list of format strings ordered according to arg labels:  
        ['%6d', '%s', '%8.3f']
        - dictionary where keys are labels and values are format strings:
        {'some ints' : '%6d', 'strings' : '%s', 'floats' : '%8.3f'}
        - 'auto': floats are formated as '%12.6f' and ints as '%12d'.
      - header: list of header lines, from the beginning to (not including) 
      the labels
      - tablename: table name ('data_' for relion star files), ignored if
      arg header is specified
      - labelindent: (str) label indentation (typically ' ' for xmipp, None
      for relion
      - delimiter: column delimiter, not used is format is a single string
      - encoding: text encoding, used only in python3, default 'ascii'
    """
    ...
    # labels
    for label, ind in zip(labels, list(range(len(labels)))):
    ...
    for one_line in lines:
        try:
            # python 3+
            tmp_line = bytes(one_line + os.linesep, encoding=encoding)
            fd.write(tmp_line)
        except TypeError:
            # python 2
            tmp_line = one_line + os.linesep
            fd.write(tmp_line)
...
def symmetrize_particle_data(
    ...
    for init_data, labels in init_iter:
        init_dict = dict(list(zip(labels, init_data)))
        ...
        for sym_euler, sym_ind in zip(angle_iter, list(range(len(angle_iter)))):
...
def add_priors(instar, outstar, prior_labels=None, format_='auto'):
    ...
    for init_data, labels in init_iter:
        ...
        for lab in labels:
            ...
            if lab in list(prior_labels.keys()):
    ...
    for lab in labels:
        ...
        if not priors_added and (lab in pls):
            ...
            if len(pls) == 0:
                ...
                # add prior labels
                for pri_lab in list(prior_labels.values()):
...
def symmetrize_structure(
    ...
    # origin
    if origin is None:
        origin = old_div(np.asarray(array.shape), 2) 
    
    new_array = np.zeros_like(array)
    for sym_euler, sym_ind in zip(angle_iter, list(range(sym_multiplicity))):  
