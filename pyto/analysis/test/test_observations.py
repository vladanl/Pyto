...
class TestObservations(np_test.TestCase):
    ...
    @classmethod
    def makeInstance_no_categs(cls):
        """
        """
        obs = Observations()
        obs.identifiers = ['exp_1', 'exp_2', 'exp_5', 'exp_6']
        ...
        obs.scalar = [1, 2, 5, 6]
        obs.indexed = set(['ids', 'mean', 'pair'])
        obs.properties = set(['ids', 'identifiers', 'mean', 'scalar', 'pair'])
    ...
    def test_get_indexed_data(self):
        ...
        # no iddexed data saved as None (and not as [])
    ...
    def testAddExperiment(self):
        ...
        np_test.assert_equal(
            new.identifiers, ['exp_1', 'exp_2', 'exp_5', 'exp_6'])
        np_test.assert_equal(
            new.properties,
            set(['ids', 'identifiers', 'categories', 'mean',
                 'scalar', 'pair']))
    ...
    def testJoinExperiments(self):
        ...
        np_test.assert_equal('scalar' in exp.indexed, True)

        # scalar property first, mode join, identifiers
        exp = self.obs.joinExperiments(
            name=['scalar', 'mean'], mode='join',
        ...
        np_test.assert_equal(
            set(exp.properties),
            set(['identifier', 'scalar', 'mean', 'ids', 'idNames']))
        np_test.assert_equal(
            set(exp.indexed), set(['ids', 'idNames', 'scalar']))

        # indexed property first, mode join, identifiers
        exp = self.obs.joinExperiments(
            name=['mean', 'scalar'], mode='join',
        ...
        np_test.assert_equal(
            ...
            set(['identifier', 'scalar', 'mean', 'ids', 'idNames']))
        np_test.assert_equal(
            set(exp.indexed), set(['ids', 'idNames', 'mean']))

        # indexed property, mode mean
        exp = self.obs.joinExperiments(name='mean', mode='mean')
        np_test.assert_equal(
            exp.properties, set(['identifier', 'ids', 'idNames', 'mean']))
        np_test.assert_equal(
            exp.indexed, set(['ids', 'idNames', 'mean']))
        np_test.assert_equal(exp.idNames, ['exp_1', 'exp_2', 'exp_6'])
        np_test.assert_equal(exp.mean, [6, 4, 12])
        np_test.assert_equal('mean' in exp.indexed, True)
    ...
    def testSetValue(self):
       ...
       # start from empty object, add non-indexed
        ...
        # Test the fix for the problem that occurs when a scalar property
        # ('prop') is added (setValue()) to an Observations object (obs) that
        # has obs.prop = [], but does not have 'prop' in obs.properties.
        # This was the case for property 'categories' of newly instantiated
        # Observations() (8.2021).
        obs = self.makeInstance_no_categs()
        obs.setValue(
            name='categories', identifier='exp_2', value='group_2',
            default='dummy')
        np_test.assert_equal('categories' in obs.properties, True)
        np_test.assert_equal(
            obs.getValue(name='categories', identifier='exp_2'), 'group_2')
        np_test.assert_equal(
            obs.getValue(name='categories', identifier='exp_5'), 'dummy')
    ...
    def test_scalar_to_indexed(self):
        """
        Tests scalar_to_indexed()
        """

        # without identifiers
        obs = self.makeInstance()
        obs.scalar_to_indexed(scalar='scalar', indexed='sc2in')
        np_test.assert_equal('sc2in' in obs.properties, True)
        np_test.assert_equal('sc2in' in obs.indexed, True)
        np_test.assert_equal(
            obs.getValue(identifier='exp_1', name='sc2in'), [1, 1, 1])
        np_test.assert_equal(
            obs.getValue(identifier='exp_2', name='sc2in'), [2, 2, 2])
        np_test.assert_equal(
            obs.getValue(identifier='exp_5', name='sc2in'), [])
        np_test.assert_equal(
            obs.getValue(identifier='exp_6', name='sc2in'), [6])

        # with identifiers
        obs = self.makeInstance()
        obs.scalar_to_indexed(
            scalar='scalar', indexed='sc2in', identifiers=['exp_5', 'exp_6'])
        np_test.assert_equal('sc2in' in obs.properties, True)
        np_test.assert_equal('sc2in' in obs.indexed, True)
        np_test.assert_equal(
            obs.getValue(identifier='exp_1', name='sc2in'), [None, None, None])
        np_test.assert_equal(
            obs.getValue(identifier='exp_2', name='sc2in'), [None, None, None])
        np_test.assert_equal(
            obs.getValue(identifier='exp_5', name='sc2in'), [])
        np_test.assert_equal(
            obs.getValue(identifier='exp_6', name='sc2in'), [6])

        # with nonexisting identifiers
        obs = self.makeInstance()
        obs.scalar_to_indexed(
            scalar='scalar', indexed='sc2in',
            identifiers=['exp_88', 'exp_5', 'exp_6'])
        np_test.assert_equal('sc2in' in obs.properties, True)
        np_test.assert_equal('sc2in' in obs.indexed, True)
        np_test.assert_equal(
            obs.getValue(identifier='exp_1', name='sc2in'), [None, None, None])
    ...
    def test_is_scalar(self):
        ...
        obs = self.makeInstance()
        np_test.assert_equal(obs.is_scalar('scalar'), True)
        np_test.assert_equal(obs.is_scalar('mean'), False)
        np_test.assert_equal(obs.is_scalar('nonexisting'), None)
    ...
    def test_is_indexed(self):
        ...
        obs = self.makeInstance()
        np_test.assert_equal(obs.is_indexed('scalar'), False)
        np_test.assert_equal(obs.is_indexed('mean'), True)
        np_test.assert_equal(obs.is_indexed('nonexisting'), None)

    def test_remove_property(self):
        """
        Tests remove_property()
        """

        obs = self.makeInstance()
        obs.remove_property(name='mean')
        np_test.assert_equal(obs.is_indexed('mean'), None)
        np_test.assert_equal(obs.is_scalar('mean'), None)
        np_test.assert_equal(getattr(obs, 'mean', 'not found'), 'not found')
        obs.remove_property(name='scalar')
        np_test.assert_equal(obs.is_indexed('scalar'), None)
        np_test.assert_equal(obs.is_scalar('scalar'), None)
        np_test.assert_equal(getattr(obs, 'scalar', 'not found'), 'not found')
    ...
    def testDoInference(self):
        ...
        np_test.assert_almost_equal(
                ...
                numpy.nan, decimal=3)
        np_test.assert_equal(
            numpy.isnan(stats2.getValue(
                property='testValue', identifier='exp_5')),
            True)
        np_test.assert_equal(stats2.reference, ['exp_1', 'exp_1', 'exp_1'])

        # single reference not in identifiers
        some_ident = ['exp_11', 'exp_6', 'exp_5']
        stats2 = obs.doStats(name='mean', new=True, identifiers=some_ident)
        reference='exp_1'
        np_test.assert_raises(
             ReferenceError, stats2.doInference, test='t', reference=reference,
             identifiers=some_ident)      
        ...
        # reference dictionary, reference not in identifiers
        reference={'exp_1':'exp_2', 'exp_2':'exp_1',
                   'exp_5':'exp_2', 'exp_6':'exp_2'}
        some_ident = ['exp_1', 'exp_5', 'exp_6']
        np_test.assert_raises(
             ReferenceError, stats.doInference, test='t', reference=reference,
             identifiers=some_ident)              
        ...
        # reference list, reference not in identifiers
        reference = ['exp_2', 'exp_1', 'exp_2']
        some_ident = ['exp_1', 'exp_5', 'exp_6']
        np_test.assert_raises(
             ReferenceError, stats.doInference, test='t', reference=reference,
             identifiers=some_ident)              
    ...
    def testDoCorrelation(self):
        ...
        np_test.assert_equal(
            obs.properties,
            set(['ids', 'identifiers', 'categories', 'mean', 'mean_2',
                 'scalar', 'pair']))
        np_test.assert_equal(
            obs.indexed, set(['ids', 'mean', 'pair', 'mean_2']))
        np_test.assert_equal(
            corr.properties,
        ...
        for ind in [0, 1]:
            np_test.assert_almost_equal(corr.testValue[ind], 1)
            np_test.assert_almost_equal(
        ...
        corr = obs.doCorrelation(
            xName='mean', yName='mean_2', mode=None, test='r', new=True,
        ...
        for ind in [0, 1]:
            np_test.assert_almost_equal(obs.testValue[ind], 1)
            np_test.assert_almost_equal(
        ...
        # mode None, scalar
        np_test.assert_raises(
            ValueError, obs.doCorrelation, xName='mean', yName='scalar')
        ...
        # mode 'join', scalar and indexed
        obs = self.makeInstance()
        corr = obs.doCorrelation(
            xName='scalar', yName='mean', mode='join',
            test='r', new=True, out=out)
        x_desired = [1, 1, 1, 2, 2, 2, 6]
        y_desired = [2, 6, 10, 2, 4, 6, 12]
        np_test.assert_equal(corr.xData, x_desired)
        np_test.assert_equal(corr.yData, y_desired)
        desired = scipy.stats.pearsonr(x_desired, y_desired)
        np_test.assert_almost_equal(corr.testValue, desired[0])
        np_test.assert_almost_equal(corr.confidence, desired[1])
        np_test.assert_equal(
            obs.properties,
            set(['ids', 'identifiers', 'categories', 'mean', 'scalar', 'pair']))
        np_test.assert_equal(obs.indexed, set(['ids', 'mean', 'pair']))
        np_test.assert_equal(obs.scalar, [1, 2, 5, 6])
        np_test.assert_equal(
            obs.mean,
            [numpy.array([2., 6, 10]), numpy.array([2., 4, 6]),
             numpy.array([]), numpy.array([12])])

        # mode 'join', x and y names reversed
        corr = obs.doCorrelation(
            xName='mean', yName='scalar', mode='join',
            test='r', new=True, out=out)
        np_test.assert_equal(corr.xData, y_desired)
        np_test.assert_equal(corr.yData, x_desired)
        np_test.assert_almost_equal(corr.testValue, desired[0])
        np_test.assert_almost_equal(corr.confidence, desired[1])
        np_test.assert_equal(
            obs.properties,
        ...
        # mode mean, scalar and indexed
        corr = obs.doCorrelation(
            xName='mean', yName='scalar', mode='mean', test='r',
            new=True, out=out)
        x_desired = [6, 4, 12]
        y_desired = [1, 2, 6]
        np_test.assert_equal(corr.xData, x_desired)
        np_test.assert_equal(corr.yData, y_desired)
         ...
         # mode mean, scalar and indexed reversed
        corr = obs.doCorrelation(
            yName='mean', xName='scalar', mode='mean', test='r',
            new=True, out=out)
        x_desired = [6, 4, 12]
        y_desired = [1, 2, 6]
        np_test.assert_equal(corr.xData, y_desired)
        ...
        corr = obs.doCorrelation(
            xName='mean', yName='mean', mode='mean', test='r',
            new=True, out=out)
        x_desired = [6, 4, 12]
        y_desired = [6, 4, 12]
        np_test.assert_equal(corr.xData, x_desired)
