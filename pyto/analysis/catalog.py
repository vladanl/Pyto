...
try:
    ...
    warnings.warn(
        "Module importlib not found, loading imp instead "
...
class Catalog(object):
    ...
    def __init__(
            self, catalog=None, dir='.', type='distributed', identifiers=None):
        """
    ...
    def read(self, catalog, dir='.', type='distributed', identifiers=None,
        ...
        if type == 'single':
            ...
            self._readDistributed(
                catalog=catalog, dir_=dir,
                identifiers=identifiers, extensions=extensions)

    def _readDistributed(
            self, catalog, dir_='.', extensions=['.pkl', '.dat'],
            identifiers=None):
        """
        ...
        for file_ in os.listdir(dir_):
            for cat in catalog:
                ...
                cat_path = os.path.normpath(os.path.join(dir_, file_))
                ...
                try:
                    ...
                    if 'imp' in sys.modules:
                        ...
                        try:
                            ...
                            # happens when run from a command line, don't
                            # know why
                            mod_file, mod_dir, mod_desc = imp.find_module(
                        ...
                        raise ValueError(
                            "Neither imortlib nor imp module is available")
    ...
    def get_path(self, name, start_dir='.'):
        """

        """

        path_this = getattr(self, name)
        path_rel = os.path.relpath(path_this, start_dir)
