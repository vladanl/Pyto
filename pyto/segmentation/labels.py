...
class Labels(Image):
    ...
    segments. However, it is not defined how exactly are segments represented
    ...
    # numerical factor used in _remove method. The current value is used 
    # because 'remove' mode is 20% faster than the 'keep mode'
        ...
        try:
            ...
            for ins_list in insets:
                if ins_list is None:
                    continue
                inset_array.append(
                    [[ins.start, ins.stop] for ins in ins_list])
    ...
    def remove(self, ids, value=0):
        ...
        the data array (self.data) and ids (self.ids). Thus it modifies 
        self.data.
        ...
        if mode == 'remove':
            ...
            for id in remove:
                ...
                if curr_slice is None:
                    continue
                condition[curr_slice] = (
                    condition[curr_slice] | (data[curr_slice] == id))
    ...
    def orderIdsByValues(self, values, contacts=None, argsort=False):
        ...
        permutation that orders values is then used to order ids.
